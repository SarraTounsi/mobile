...
public class CartService {
    ...
    /*public void AddToCart(Product product){
        ...
        String url = Statics.BASE_URL+"/api/panier/new2";
    ...
    public ArrayList<Commande> getCommands(){
        ...
        ConnectionRequest request = new ConnectionRequest();
        String url = Statics.BASE_URL+"/api/commande/afficheClient/52";
        request.setUrl(url);
    ...
    private ArrayList<Commande> parseEvent(String jsonText) throws IOException, NoSuchFieldException, IllegalAccessException, ParseException {
        ...
        for (Map<String,Object> obj : list){
            ...
            String ref = obj.get("avisClient").toString();
            String idClient = obj.get("idClient").toString();
            Commande commande = new Commande();

            commande.setId(comId);
            commande.setAvisClient(ref);
            commande.setIdClient("52");
    ...
    public boolean pay(String address) {
        ...
        status.show();
        String url = Statics.BASE_URL+"/api/panier/validerPanier";
        ConnectionRequest request = new ConnectionRequest();
        request.setUrl(url);
        request.setPost(true);
        //request.addArgument("date", address);
        //request.addArgument("idClient", "52");
        request.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
                resultOK = request.getResponseCode() == 200;
    ...
    public boolean addToDataBaseCart(Product product) {
        ...
        request.setPost(true);
        request.addArgument("product", product.getId());
        request.addArgument("id_client", "52");
    ...
    public void updateCommands(int selectedStringIndex, Object o) {
        ...
        String url = Statics.BASE_URL+"/api/commande/editEtat/"+ o.toString();
        ConnectionRequest request = new ConnectionRequest();
        request.setUrl(url);
        request.setPost(true);
        System.out.println(o);
    ...
    public void updateCartItemQuantity(String quantity,String id) {
        ...
        status.setShowProgressIndicator(true);
        status.show();
        String url = Statics.BASE_URL+"/api/panier/editQuantity/"+ id;
        ConnectionRequest request = new ConnectionRequest();
        request.setUrl(url);
        request.setPost(true);
